...
static int
ensure_comm_capacity_bson(struct testing_report_test *p, int need_count)
{
    if (!p || need_count <= 0)
        return -1;
...
parse_communication_run_doc(bson_iter_t *bi, struct testing_report_test *p)
{
    ...
    while (bson_iter_next(bi)) {
        ...
        case Tag_input:
            trfc = NULL;
            if (num <= 0) return -1;
            if (ensure_comm_capacity_bson(p, num) < 0)
                return -1;
    ...
    if (r->run_tests > 0 && r->tests) {
        ...
        for (int i = 0; i < r->run_tests; ++i) {
            ...
            if (t->communication_run_count > 0) {
                ...
                int cindex = -1;
                for (int ci = 1; ci <= t->communication_run_count; ++ci) {
                    ++cindex;
                    bson_t b_comm;
                    {
                        char buf[32];
                        const char *key = NULL;
                        uint32_t z = bson_uint32_to_string(cindex, &key, buf, sizeof(buf));
                        bson_append_document_begin(&b_comms, key, z, &b_comm);
                    }
