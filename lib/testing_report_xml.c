...
static int
ensure_comm_capacity(struct testing_report_test *p, int run_idx)
{
        ...
        for (struct xml_attr *a3 = t3->first; a3; a3 = a3->next) {
          switch (a3->tag) {
            ...
            case TR_A_TIME:
              if (xml_attr_int(a3, &x) < 0 || x < 0) goto failure;
              if (num > 0) {
                if (ensure_comm_capacity(q, num) < 0) goto failure;
      ...
      if (t->communication_run_count > 0) {
        ...
        for (int c = 1; c <= t->communication_run_count; ++c) {
          ...
          if (t->communication_inputs) {
            unparse_file_content(out, &ab, TR_T_COMMUNICATION_INPUTS, &t->communication_inputs[c]);
          }
          if (t->communication_outputs) {
            unparse_file_content(out, &ab, TR_T_COMMUNICATION_OUTPUTS, &t->communication_outputs[c]);
          }
