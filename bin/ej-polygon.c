...
static unsigned char *
make_comm_flags(int run_count, int value)
{
...
static int
set_comm_flag(unsigned char *flags, int run_count, int run_num, int value)
{
        ...
        struct ProblemInfo *pi)
{
    ...
    if (ppx->judging) {
        ...
        if (ppj->run_count >= 2) {
            ...
            xfree(pi->communication_flags);
            pi->communication_flags = make_comm_flags(ppj->run_count, 0);
        }
        ...
        if (ppa->interactor) {
            ...
            if (pi->communication >= 2) {
                if (!pi->communication_flags) {
                    pi->communication_flags = make_comm_flags(pi->communication, 0);
                }
                int has_runs = 0;
                if (ppa->interactor->runs && ppa->interactor->runs->n.u > 0) {
                    ...
                    for (size_t i = 0; i < ppa->interactor->runs->n.u; ++i) {
                      ...
                      if (set_comm_flag(pi->communication_flags, pi->communication, rr->value, 1) < 0) {
                        fprintf(log_f, "interactor run %d is out of range [1..%d]\n", rr->value, pi->communication);
                        goto done;
                      }
                    }
                } else if (pi->communication == 2) {
                    if (set_comm_flag(pi->communication_flags, pi->communication, 1, 1) < 0) {
                        fprintf(log_f, "failed to mark first run as interactive\n");
                        goto done;
                    }
                }
            }
    ...
    for (struct xml_tree *t1 = tree->first_down; t1; t1 = t1->right) {
        if (!strcmp(t1->name[0], "names")) {
            ...
            if (run_count >= 2) {
                ...
                xfree(pi->communication_flags);
                pi->communication_flags = make_comm_flags(run_count, 0);
            }
            for (struct xml_tree *t2 = t1->first_down; t2; t2 = t2->right) {
            ...
            for (struct xml_tree *t2 = t1->first_down; t2; t2 = t2->right) {
                if (!strcmp(t2->name[0], "checker")) {
                        ...
                        if (!strcmp(t3->name[0], "source")) {
                            ...
                            for (struct xml_tree *t4 = t3->first_down; t4; t4 = t4->right) {
                                ...
                                if (pi->communication >= 2) {
                                    ...
                                    if (set_comm_flag(pi->communication_flags, pi->communication, run_num, 1) < 0) {
                    ...
                    if (pi->communication >= 2) {
                        ...
                        if (!has_runs && pi->communication == 2) {
                            set_comm_flag(pi->communication_flags, pi->communication, 1, 1);
                        }
                    }
                    if (!src_path) {
